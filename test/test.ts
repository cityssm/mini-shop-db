  ...
  items: [
    {
      ...
      quantity: 4,
      unitPrice: testProduct.price as number,
      itemTotal: 4 * (testProduct.price as number),
...
await describe('mini-shop-db', async () => {
  ...
  await it('fails to refund an unpaid order', async () => {
    const success = await miniShopDB.updateOrderAsRefunded(
      orderNumber,
      orderSecret,
      {
        refundID: '-1',
  ...
  await it('runs isOrderFoundAndPaid() - finds order as unpaid', async () => {
    const result = await miniShopDB.isOrderFoundAndPaid(
      orderNumber,
      orderSecret
    )
  ...
  await it('runs getOrderNumberBySecret()', async () => {
    const databaseOrderNumber =
      await miniShopDB.getOrderNumberBySecret(orderSecret)
